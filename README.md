...
**Authorization layer (v1.3, on by default).** A central policy gate adds least-privilege on top of authentication: **roles** (admin / user / viewer), **deny-by-default tool permissions** (registration, secrets and identity tools are admin-only), an **audit log** of decisions, and **per-credential identity binding**. It's **secure by default** (`AUTH_ENFORCE=0` disables it): the headless `RUNNER_TOKEN` is a non-admin **user** while an interactive OIDC operator is **admin** — so the autonomy runner can't register services, add cron jobs or set secrets, and `mail_send` honours a recipient allow-list. Roles can be driven from an **IdP role/group claim** (`AUTH_ROLE_CLAIM`, e.g. Pocket ID groups) the moment the OIDC layer forwards it, or set in `data/auth/policy.json`. Full multi-user / groups / per-user API keys are on the v2.x roadmap. **Setup & usage: [docs/authorization.md](docs/authorization.md).**
